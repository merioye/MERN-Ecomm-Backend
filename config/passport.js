...
const authUser = async (request, accessToken, refreshToken, profile, done) => {
    try{
        if(!profile.emails[0]){
            return done(null, { error: true, errorText: `Facebook id's created using phone no cannot be used to login here, please choose another method to login` });
        }

        const user = await UserModel.findOne({ email: profile.emails[0].value }).populate('address');
        if(user && user.method!==profile.provider){ 
            const method = user.method==='custom' ? 'email & password' : user.method; 
            ...
            return done(null, { error: true, errorText: `Same email has been already used to login with ${method}, please select a different email OR login with ${method}` });
        }
        ...
        console.log(e);
        return done(null, { error: true, errorText: 'Something went wrong, please try again' });
    }
}
