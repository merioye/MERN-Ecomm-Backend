...
class CartController {
    getCart = async (req, res, next) => {
        try {
            ...
            let cart = await CartModel.findById({ _id: _id }).populate(
                "products.product",
                "name images stock regularPrice salePrice isOnSale"
            );
    ...
    updateCart = async (req, res, next) => {
        try {
            ...
            if (!req.body)
                return next(createError(400, "Oops! some problem occurred"));
            ...
            if (updatedProducts.length) {
                ...
                const cart = await CartModel.findById({ _id }).select(
                    "couponMinimumAmount -_id"
                );
                if (
                    cart &&
                    Number(orderTotalAmount) < cart.couponMinimumAmount
                ) {
    ...
    applyCouponCode = async (req, res, next) => {
        try {
            ...
            if (!voucherCode || !purchaseAmount)
                return next(
                    createError(
                        400,
                        "Voucher code or Purchase amount is missing"
                    )
                );
            ...
            if (!coupon)
                return next(createError(404, "Voucher code is invalid"));
            ...
            if (cart)
                return res.status(200).json({
                    ...
                    message: "Voucher has been already applied",
                });

            if (Number(purchaseAmount) < coupon.minimumAmount)
                return next(
                    createError(
                        406,
                        `Please make an order of at least $${coupon.minimumAmount} to avail this Voucher`
                    )
                );

            if (new Date(coupon.validity).getTime() < new Date().getTime())
                return next(createError(410, "Voucher code has been expired!"));

            const dataToUpdate = {
                couponCode: coupon.couponCode,
                couponDiscountPercentage: coupon.discountPercentage,
                couponMinimumAmount: coupon.minimumAmount,
            };
    ...
    getCheckoutData = async (req, res, next) => {
        try {
            ...
            const cart = await CartModel.findById({ _id: _id }).populate(
                "products.product",
                "name images stock regularPrice salePrice isOnSale"
            );
            if (!cart) next(createError(404, "Cart data not found!"));
    ...
    getPaymentPageData = async (req, res, next) => {
        try {
            const { _id } = req.userData;
            ...
            if (!userAddress) return res.status(200).json({ error: true });
