...
class ProductController {
    ...
    topRatedsorter = (p1, p2) => {
        const p1AvgRating =
            p1.reviews.reduce((acc, review) => acc + review.rating, 0) /
            p1.reviews.length;
        const p2AvgRating =
    ...
    getHomeData = async (req, res, next) => {
        try {
            ...
            const featuredBrands = brandsData
                ...
                .slice(0, 8);
            const flashDealProducts = productsData
            ...
            const moreForYouProducts = productsData
                .filter((p) => !allProducts.includes(p))
                .slice(0, 8); // products excluding allProducts

            // array of prices of all the products in sorted order
            const productsPrices = productsData.map((p) =>
                p.isOnSale ? p.salePrice : p.regularPrice
            );
            ...
            res.status(200).json({
                ...
                productsPriceRange: {
                    min: productsPrices[0],
                    max: productsPrices[productsPrices.length - 1],
                },
                featuredBrands,
                flashDealProducts,
                featuredProducts,
    ...
    getFilteredProducts = async (req, res, next) => {
        try {
            ...
            if (isBrand) {
                let brandsData = await redisData.getBrands();
                const featuredBrands = brandsData.filter(
                    (brand) => brand.isFeatured === true
                );
                ...
                if (Array.isArray(rating)) {
                    productsData = productsData.filter((p) => {
                        ...
                        return rating.includes(
                            String(Math.trunc(productAverageRating))
                        );
                    });
                } else {
                    productsData = productsData.filter((p) => {
                        ...
                        return (
                            String(Math.trunc(productAverageRating)) === rating
                        );
                    });
                }
            }
            ...
            if (price) {
                ...
                productsData = productsData.filter((p) => {
                    if (p.isOnSale) {
                        return (
                            p.salePrice >= Number(minAndMaxPrice[0]) &&
                            p.salePrice <= Number(minAndMaxPrice[1])
                        );
                    } else {
                        return (
                            p.regularPrice >= Number(minAndMaxPrice[0]) &&
                            p.regularPrice <= Number(minAndMaxPrice[1])
                        );
                    }
                });
            }
            if (sort) {
                if (sort === "date") {
                    ...
                    productsData.sort((p1, p2) => {
                        return (
                            (p1.isOnSale ? p1.salePrice : p1.regularPrice) -
                            (p2.isOnSale ? p2.salePrice : p2.regularPrice)
                        );
                    });
                } else if (sort === "htl") {
                    productsData.sort((p1, p2) => {
