    ...
    getValuesFromList = async (key, toSkip, toFetch, search)=>{
        try{
            ...
            if(search){
                ...
                const matchedValues = parsedValues.filter(val => val.name.toLowerCase().includes(search.toLowerCase()));
                values = matchedValues.slice(toSkip, (toSkip+toFetch));
                matchedCount = matchedValues.length;
            }else{
                const stringifyValues = await redisClient.LRANGE(key, toSkip, (toSkip+toFetch)-1);
                values = stringifyValues.map(val => JSON.parse(val));
            }
            const totalValues = await redisClient.LLEN(key);
            return { totalCount: totalValues, values: values, matchedCount: matchedCount };
        }catch(e){
            throw e;
        }
    }
